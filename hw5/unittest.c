...
int main() {
    ...
    testCompareCourse();
    testSearchCourse();
    testSearchTranscript();
    testSearchTranscriptNoDel();
...
void testSearchTranscript(){
    node *head = Create(data[0], "1000.s", "A+");
    assert(searchTranscript(&head, data[0]) == 1);
}
void testSearchTranscriptNoDel(){
    node *head = Create(data[0], "1000.s", "A+");
    assert(searchTranscriptNoDel(&head, data[0]) == 1);
}
...
void testCheckDup(){
    node *head = Create(data[0], "1000.s", "A+");
    assert(checkDup(&head, data[0], "1000.s") == 1);
}
